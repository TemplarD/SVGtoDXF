...
- ✅ **Растровые изображения** (`image`, PNG/JPEG/GIF) — **трассируются в вектор** через marching squares (изолинии по яркости → набор LWPOLYLINE). Вложенный **SVG через data URI** (`<image href="data:image/svg+xml,...">`) пока не поддерживается.
