...
### Интерфейс
- 🎨 Тёмная тема, компактный адаптивный дизайн (нет боковых полей, вписывается в любое окно)
- 🔔 **Анимация-подсказка кнопки «Конвертировать»** — пока не выбраны SVG и/или папка вывода, кнопка пульсирует и рядом появляется стрелочка 👉 с точным действием («Выберите SVG файлы…» / «Выберите папку вывода»)
- ⚙️ Панель настроек (tooltip на hover):
  ...
  - **Точные цвета (true‑color)** — только при включённом «Сохранять цвета»
  - **Трассировка растра** — PNG/JPEG/GIF → вектор
- 📋 Список файлов → статус (✓/✗) в реальном времени
- 📊 **Прогресс‑бар конвертации** — полоса с текстом «N / M (XX%)», видимая только во время конвертации
- 💾 **Сохранение папок между запусками** — выходная папка и последняя папка выбора SVG запоминаются и подставляются при следующем старте (рядом с exe в portable‑режиме либо в конфиге приложения)

### Имена выходных файлов
- 🔁 **Чекбокс «Заменять файлы»** рядом с кнопкой конвертации:
  - *Выключен (по умолчанию)* — при совпадении имени создаётся `drawing_1.dxf`, `drawing_2.dxf`, … (существующие не перезаписываются)
  - *Включен* — перезаписывает
- 🏷️ При включённых «Сохранять цвета» / «Заливка линиями» становятся доступны чекбоксы **`+ _color`** / **`+ _hatch`**, добавляющие к имени суффикс (`drawing_color.dxf`, `drawing_hatch.dxf`)
...
| Формат | Путь |
|--------|------|
| **NSIS‑installer** (с установкой) | `releases/windows/SVG to DXF Converter_1.0.0_x64-setup.exe` |
| **Портативная** (без установки, zip) | `releases/windows/SVGtoDXF-Portable-x64.zip` |

> **Портативная версия** — просто распакуйте zip и запустите `svg2dxf-tauri-app.exe`. Ничего не устанавливается, можно переносить на флешке. Настройки папок сохраняются рядом с exe (portable‑режим). Собирается скриптом `build/portable-windows.sh` (Linux) или `build/portable-windows.ps1` (Windows).
...
```bash
cargo tauri build                                 # Linux
cargo tauri build --target x86_64-pc-windows-gnu  # Windows (инсталлятор + portable)
bash build/collect_releases.sh                    # копирует всё в releases/
